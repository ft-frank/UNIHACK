...
export default function App() {
  ...
  const handleLoadVideo = async (e: React.FormEvent) => {
    ...
    // Fetch questions in background with settings
    setLoading(true);
    try {
    ...
    <div className="min-h-screen bg-white text-slate-800 font-sans">
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 flex flex-col">
          ...
          {loading && (
            <div className="mt-4 flex items-center gap-2 text-sm text-gray-500">
              <div style={{
                ...
                borderTopColor: "#334155", borderRadius: "50%",
          ...
          {!loading && <p className="text-sm text-gray-500 mt-4 font-medium">Pause the video to reveal quiz questions</p>}
        </div>
          ...
          <div className="border border-gray-200 rounded-xl bg-slate-50/50 p-8 flex flex-col justify-center min-h-[160px]">
            {currentQuestion ? (
              ...
              <p className="text-center text-slate-500/80 font-medium text-sm">
                {loading ? "Questions are being generated…" : "Pause the video to start answering questions"}
              </p>
            )}
          </div>
        </div>
